...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $query = "SELECT * FROM kritik_pelanggan WHERE id_umpan_balik = '$id'";
...
<body>
        ...
        <div class="flex-grow text-gray-800">
            ...
            <main class="p-6 sm:p-10 space-y-6">
                <div
                    ...
                    <div class="mr-6">
                        <h1 class="text-4xl font-semibold mb-2">Edit Report</h1>
                        <h2 class="text-gray-600 ml-0.5">Edit Report: Memodifikasi Sebuah Kritik & Saran</h2>
                    </div>
                ...
                <section class="grid md:grid-cols-1 xl:grid-cols-1 gap-6">
                    <div class="relative overflow-x-auto border-4 shadow-md sm:rounded-lg w-full mx-auto my-24 ">        

                        <form method="POST" enctype="multipart/form-data">
                            <!-- Input Hidden untuk ID -->
                            <input type="hidden" name="id" value="<?= $content['id_umpan_balik']; ?>">
                            <!-- Input Hidden untuk gambar lama -->
                            <input type="hidden" name="current_image" value="<?= $content['image']; ?>">

                            <div class="grid gap-6 mb-6 md:grid-cols-2 max-w-6xl mx-auto my-12">
                                <div>
                                    <label for="first_name" class="block mb-2 text-sm font-medium text-gray-900 ">Nama</label>
                                    <input name="nama" type="text" id="first_name" value="<?= $content['nama']; ?>" class="bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Input nama anda" required />
                                </div>
                                <div>
                                    <label for="last_name" class="block mb-2 text-sm font-medium text-gray-900 ">Email</label>
                                    <input name="email" type="email" id="last_name" value="<?= $content['email']; ?>" class="bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Input email anda" required />
                                </div>
                                <div class="col-span-2">
                                    <label for="countries" class="block mb-2 text-sm font-medium text-gray-900">Pilih Tipe Message</label>
                                    <select name="jenis_umpan_balik" id="countries" class="bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                                        <option disabled <?= empty($content['jenis_umpan_balik']) ? 'selected' : ''; ?>>Tipe Message</option>
                                        <option value="Kritik" <?= $content['jenis_umpan_balik'] == 'Kritik' ? 'selected' : ''; ?>>Kritik</option>
                                        <option value="Saran" <?= $content['jenis_umpan_balik'] == 'Saran' ? 'selected' : ''; ?>>Saran</option>
                                    </select>
                                </div>
                            </div>
                            <div class="mb-6 max-w-6xl mx-auto">
                                <label for="message" class="block mb-2 text-sm font-medium text-gray-900">Message Anda</label>
                                <textarea name="isi_umpan_balik" id="message" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-200 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Tulis kritik dan saran anda disini..." required><?= $content['isi_umpan_balik']; ?></textarea>
                            </div>
                            <div class="mb-6 max-w-6xl mx-auto">
                                <label class="block mb-2 text-sm font-medium text-gray-900" for="file_input">Upload Image</label>
                                <input name="image" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-100 dark:text-gray-400 focus:outline-none" id="file_input" type="file">
                                <!-- Tampilkan gambar lama -->
                                <?php if (!empty($content['image'])): ?>
                                    <img src="/images/<?= $content['image']; ?>" alt="Current Image" class="mt-3 rounded-lg" width="150">
                                <?php endif; ?>
                            </div>
                            <div class="mb-6 max-w-6xl mx-auto">
                                <button name="submit" type="submit" class="text-white bg-purple-600 hover:bg-purple-700 focus:ring-2 focus:outline-none focus:ring-purple-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Submit</button>
                            </div>
                        </form>
                    </div>
                </section>
    ...
    <?php if (isset($_GET['action']) && $_GET['action'] === 'edit' && isset($_GET['id'])) {
        $id = $_GET['id'];
        // Fetch data for the given ID and display the form for editing.
    } else {
        // Redirect or show an error if the action or ID is missing.
